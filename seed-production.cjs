...
async function seedProduction() {
  ...
  try {
    // Check if puppies already exist
    const existingPuppies = await prisma.puppy.count();
    console.log(`📊 Current puppies in database: ${existingPuppies}`);

    if (existingPuppies > 0) {
      console.log('✅ Production database already has puppies, skipping seed');
      return;
    }

    // Create breeds (same as local seed)
    ...
    // Create puppies (same as local seed)
